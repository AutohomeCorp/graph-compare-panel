...
import CustomScrollbar from '../vendor/grafana/customscrollbar/CustomScrollbar'
...
export class GraphLegend extends PureComponent<GraphLegendProps, LegendState> {
  ...
  render() {
    ...
    const seriesList = this.sortLegend().filter(
      series => !series.hideFromLegend(seriesHideProps)
    )
    const legendClass = `${
      this.props.alignAsTable ? 'graph-legend-table' : ''
    } ${optionalClass}`
    ...
    return (
      ...
      >
        {this.props.alignAsTable ? (
          <LegendTable {...legendProps} />
        ) : (
          <LegendSeriesList {...legendProps} />
        )}
      </div>
    )
  }
...
class LegendTableHeaderItem extends PureComponent<
  LegendTableHeaderProps & LegendSortProps
> {
  ...
  render() {
    ...
    return (
      <th className="pointer" onClick={this.onClick}>
        {statName}
        {sort === statName && (
          <span className={sortDesc ? 'fa fa-caret-down' : 'fa fa-caret-up'} />
        )}
      </th>
    )
  }
