...
export class GraphLegend {
  ...
  render() {
    ...
    var width =
      this.panel.legend.rightSide && this.panel.legend.sideWidth
    ...
    this.$elem.toggleClass(
      'graph-legend-table',
      this.panel.legend.alignAsTable === true
    )
    ...
    if (this.panel.legend.sort) {
      this.seriesList = _.sortBy(
        this.seriesList,
        series => series.stats[this.panel.legend.sort]
      )
  ...
  addScrollbar() {
    ...
    if (!this.legendScrollbar) {
      this.legendScrollbar = new PerfectScrollbar(
        this.$elem[0],
        scrollbarOptions
      )
    } else {
