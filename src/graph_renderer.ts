...
import './vendor/flot/jquery.flot.orderbars.js'
...
export class GraphRenderer {
  ...
  constructor(private $elem, private timeSrv, private contextSrv, scope) {
    ...
    this.$elem.bind('plotselected', (event, selectionEvent) => {
      ...
      if (
        (selectionEvent.ctrlKey || selectionEvent.metaKey) &&
    ...
    this.$elem.bind('plotclick', (event, flotEvent, item) => {
      ...
      if (
        (flotEvent.ctrlKey || flotEvent.metaKey) &&
        this.contextSrv.isEditor
      ) {
    ...
    this.$elem.bind('plothover', (event, pos, item) => {
      ...
      pos.panelRelY =
        (pos.pageY - this.$elem.offset().top) / this.$elem.height()
  ...
  private _getMinTimeStepOfSeries(data: any) {
  ...
  private _shouldDisplaySideBySide() {
    return (
      this.panel.displayBarsSideBySide &&
      !this.panel.stack &&
  ...
  private _displaySideBySide(options) {
    let barsSeries = _.filter(
      this.sortedSeries,
      series => series.bars && series.bars.show !== false
    )
  ...
  private _buildFlotOptions(panel: any) {
  ...
  private _sortSeries(series: any, panel: any) {
  ...
  private _addTimeAxis(minTimeStep: number) {
    ...
    if (
      ...
      let generatedTicks = this._generateTicks(
        groupsAmount,
        ticks,
        min,
        max,
      ...
      if (generatedTicks.length !== 0) {
        ...
        ticks = _.map(generatedTicks, tick => {
          ...
          return [tick[0], formatDate(date, format)]
        })
      }
    }
  ...
  private _configureYAxisOptions(data) {
    ...
    if (_.find(data, { yaxis: 2 })) {
      ...
      this._configureAxisMode(
        this.flotOptions.yaxes[1],
  ...
  private get _timeMin() {
    return _.isUndefined(this.ctrl.range.from)
      ? null
      : this.ctrl.range.from.valueOf()
  }

  private get _timeMax() {
    return _.isUndefined(this.ctrl.range.to)
      ? null
